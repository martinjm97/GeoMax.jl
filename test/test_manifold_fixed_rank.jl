...
n = 5
k = 3
s = JManOpt.FixedRank(m, n, k)

@test JManOpt.dim(s) == (m + n - k) * k

@test JManOpt.dim(s) == JManOpt.typicaldist(s)

a = rand(s)
x = JManOpt.randvec(s, a)
y = JManOpt.randvec(s, a)
...
trueinner = sum(A * B)
@test isapprox(trueinner, JManOpt.inner(s, x, a, b))

x = rand(s)
v = randn(m, n)

g = JManOpt.proj(s, x, v)
...
g = JManOpt.proj(x, v)
# Displace g a little
g_disp = g + 0.01 * JManOpt.randvec(x)

# Return to the ambient representation
g = JManOpt.tangent2ambient(x, g)
g_disp = JManOpt.tangent2ambient(x, g_disp)
...
A = JManOpt.proj(x, JManOpt.tangent2ambient(x, u))
B = u
...
x = rand(s)
u = JManOpt.randvec(s,x)
@test isapprox(sqrt(JManOpt.inner(x, u, u)), norm(x, u))
...
y = rand(s)
u = JManOpt.randvec(x)
A = JManOpt.transp(x, y, u)
B = JManOpt.proj(y, JManOpt.tangent2ambient(x, u))
diff = [A[k]-B[k] for k in range(len(A))]
@test isapprox(JManOpt.norm(y, diff), 0)
...
@test isapprox(dot(w, z), JManOpt._apply_ambient(z, w))
@test isapprox(dot(w, z), JManOpt._apply_ambient((u, s, v), w))
...
@test isapprox(dot(w, z'), JManOpt._apply_ambient_transpose(z, w))
@test isapprox(dot(w, z'), JManOpt._apply_ambient_transpose((u, s, v), w))
...
u, s, v = JManOpt.tangent2ambient(x, z)
...
@test isapprox(JManOpt.norm(x, u), 1)
@test JManOpt.norm(x, u - v) > 1e-6
