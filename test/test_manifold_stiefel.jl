...
n = 2
k = 1
s = JManOpt.Stiefel(m, n, k)
#self.proj = lambda x, u: u - npa.dot(x, npa.dot(x.T, u) +
#                                     npa.dot(u.T, x)) / 2

@test JManOpt.dim(s) == 0.5 * n* (2 * m- n- 1)
...
X = rand(s)
U = randvec(s,X)
@test isapprox(JManOpt.multisym(dot(U, X')), zeros(n, n), atol=1e-10)
V = randvec(s,X)
...
# Compare the projections.
Hproj = H - multiprod(X, multiprod(JManOpt.multitransp(X), H) +
                      multiprod(JManOpt.multitransp(H), X)) / 2
@test isapprox(Hproj, JManOpt.proj(s, X, H))

# Just make sure that things generated are on the manifold and that
# if you generate two they are not equal.
X = rand(s)
@test isapprox(multiprod(JManOpt.multitransp(X), X), multieye(k, n), atol=1e-10)
Y = rand(s)
@test vecnorm(X - Y) > 1e-6
...
X = rand(s)
U = randvec(s,X)
@test isapprox(JManOpt.multisym(multiprod(JManOpt.multitransp(X), U)), zeros(k, n, n),atol=1e-10)
V = randvec(s,X)
...
@test isapprox(multiprod(JManOpt.multitransp(xretru), xretru), multieye(s.k, n), atol=1e-10)
...
xexpu = exp(s, x, u)
@test isapprox(multiprod(JManOpt.multitransp(xexpu), xexpu), multieye(k, n), atol=1e-10)
