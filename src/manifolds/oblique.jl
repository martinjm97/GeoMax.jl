...
function dist(::Oblique, X, Y)
    XY = sum(X .* Y, 1)
    XY[XY .> 1] = 1
    return vecnorm(acos.(XY))
end

proj(::Oblique, X, H) = H .- X .* sum(X .* H, 1)
...
Base.rand(s::Oblique) = _normalize_columns(s, randn(s.m, s.n))
...
# implemented the 0 norm
_normalize_columns(::Oblique, X) = X ./ sqrt.(sum(X .!= 0, 1))
